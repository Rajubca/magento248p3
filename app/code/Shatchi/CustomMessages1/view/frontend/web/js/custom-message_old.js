define(['jquery'], function ($) {
    ...
    $(function () {
        const $stackContainer = $('<div class="custom-popup-stack" id="popup-stack"></div>');
        ...
        function updateStack() {
            $('#popup-stack').empty();
            const maxVisible = 5; // Limit visible popups to avoid clutter
            queue.slice(-maxVisible).forEach((msg, index, visibleQueue) => {
                const stackIndex = visibleQueue.length - 1 - index; // Reverse for topmost popup
                const $popup = $('<div>', {
                    class: `custom-popup ${msg.type} ${stackIndex === 0 ? '' : 'stacked'}`
                }).css({
                    top: `${30 + stackIndex * 20}px`, // Vertical offset for stack effect
                    right: `${30 + stackIndex * 10}px`, // Horizontal offset
                    transform: `rotate(${stackIndex * 2}deg)`, // Slight rotation for folder effect
                    zIndex: 9999 - stackIndex // Topmost popup has highest z-index
                }).append(
                    $(svgIcons[msg.type] || ''),
